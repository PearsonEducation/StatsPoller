...
outputfile = "sqlserver-access_poller_output.out"
If args = 1 Then
...
'****************Query Section*****************
Function GetAccess(StrSrv) 
      Dim objWMIService, Item, Proc, Time
    
      strQuery = "select * from Win32_PerfFormattedData_MSSQLSERVER_SQLServerAccessMethods"
     ...
     For Each Proc In Item
		 objFile.WriteLine "ForwardedRecords/Second " & Proc.ForwardedRecordsPersec & " " & Time
		 objFile.WriteLine "FullScans/Second " & Proc.FullScansPersec & " " & Time
		 objFile.WriteLine "IndexSearches/Second " & Proc.IndexSearchesPersec & " " & Time
		 objFile.WriteLine "PageSplits/Second " & Proc.PageSplitsPersec & " " & Time
		 objFile.WriteLine "WorkfilesCreated/Second " & Proc.WorkfilesCreatedPersec & " " & Time
		 objFile.WriteLine "WorktablesCreated/Second " & Proc.WorktablesCreatedPersec & " " & Time
		 objFile.WriteLine "TableLockEscalations/Second " & Proc.TableLockEscalationsPersec & " " & Time
    Next
End Function
'**************End Query Section***************