...
outputfile = "sqlserver-memory_poller_output.out"
If args = 1 Then
...
Function GetMemory(StrSrv) 
      ...
      strQuery = "select * from Win32_PerfFormattedData_MSSQLSERVER_SQLServerMemoryManager"
     ...
     For Each Proc In Item
		 objFile.WriteLine "GrantedWorkspaceMemory-KB " & Proc.GrantedWorkspaceMemoryKB & " " & Time
		 objFile.WriteLine "MaximumWorkspaceMemory-KB " & Proc.MaximumWorkspaceMemoryKB & " " & Time
		 objFile.WriteLine "MemoryGrantsOutstanding " & Proc.MemoryGrantsOutstanding & " " & Time
		 objFile.WriteLine "MemoryGrantsPending " & Proc.MemoryGrantsPending & " " & Time
		 objFile.WriteLine "TargetServerMemory-KB " & Proc.TargetServerMemoryKB & " " & Time
		 objFile.WriteLine "TotalServerMemory-KB " & Proc.TotalServerMemoryKB & " " & Time
    Next
End Function
'**************End Query Section***************